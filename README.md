# Personal-Projects
This is my personal projects portfolio. It includes math/physics related projects I have worked on in my free time.

## 💫 A Construction of the Orbit Space of a hamiltonian dynamical system

This small paper explores an idea I had when studying Hamiltonian mechanics. The time evolution of a system is fully determined by a point in the phase space. We know that a point in the phase space will draw a curve on the phase space over time, but the trace of such curve (the set of its points) represents *essentially the same* type of mechanical trajectory.

It would then be natural to identify all the points in a mechanical curve and collapse them into an equivalence class, yielding a new space (obtained from the phase space) which is called the Orbit Space. Here, each point corresponds to a kind of state of the sistem, we have *quotiented out time*.

As it turns out, this is a more general geometrical construction on differential manifolds: a *foliation*. I cover some interesting examples.
